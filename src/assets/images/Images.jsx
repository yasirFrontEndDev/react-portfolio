import React from 'react'

const Images = {
    logo : new URL('./site-logo.jpeg', import.meta.url).href,
    logoT : new URL('./site-logo-transparentN.png', import.meta.url).href,
    image1 : new URL('./index/hero-img.png', import.meta.url).href,
    client1 : new URL('./clients/client-1.png', import.meta.url).href,
    client2 : new URL('./clients/client-2.png', import.meta.url).href,
    client3 : new URL('./clients/client-3.png', import.meta.url).href,
    gens : new URL('./clients/99gens.svg', import.meta.url).href,
    frontEnd : new URL('./team/yasir.jpg', import.meta.url).href,
    bde : new URL('./team/bde.jpg', import.meta.url).href,
    hre : new URL('./team/hr.jpg', import.meta.url).href,
    pythonDev : new URL('./team/backend.jpg', import.meta.url).href,
    team1 : new URL('./team/team-1.jpg', import.meta.url).href,
    team2 : new URL('./team/team-2.jpg', import.meta.url).href,
    article1 : new URL('./articles/artc-1.jpg', import.meta.url).href,
    article2 : new URL('./articles/artc-2.jpg', import.meta.url).href,
    article3 : new URL('./articles/artc-3.jpg', import.meta.url).href,
    testimonial1: new URL('./testimonials/review-1.svg', import.meta.url).href,
    contactBg: new URL('./contact/contact.png', import.meta.url).href,
}

export default Images